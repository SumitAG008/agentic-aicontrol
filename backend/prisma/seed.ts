...
async function main() {
  ...
  const tools = await Promise.all([
    ...
    // ERP Integration Tools (For systems without APIs)
    prisma.tool.upsert({
      where: { slug: 'sftp-connector' },
      update: {},
      create: {
        name: 'SFTP Connector',
        slug: 'sftp-connector',
        description: 'Connect to legacy systems via SFTP file transfer',
        icon: 'folder-sync',
        ...
        inputSchema: {
          type: 'object',
          properties: {
            action: { enum: ['upload', 'download', 'list'] },
            host: { type: 'string' },
            path: { type: 'string' },
    ...
    prisma.tool.upsert({
      where: { slug: 'csv-processor' },
      update: {},
      create: {
        name: 'CSV Processor',
        slug: 'csv-processor',
        description: 'Parse and generate CSV files for ERP data exchange',
        ...
        inputSchema: {
          type: 'object',
          properties: {
            action: { enum: ['parse', 'generate', 'transform'] },
            data: { type: 'string' },
            mapping: { type: 'object' },
          },
        },
    ...
    prisma.tool.upsert({
      where: { slug: 'rpa-bridge' },
      update: {},
      create: {
        name: 'RPA Bridge',
        slug: 'rpa-bridge',
        description: 'Connect to UiPath/Automation Anywhere for legacy UI automation',
        icon: 'robot',
        category: 'INTEGRATION',
        provider: 'Generic',
        authType: 'API_KEY',
        ...
        inputSchema: {
          type: 'object',
          properties: {
            action: { enum: ['trigger_job', 'get_status', 'get_output'] },
            processKey: { type: 'string' },
            inputArguments: { type: 'object' },
          },
        },
